...
from temp.blur import approximate, svconv2d
...
class Dataset(data.Dataset):
    def __init__(self, opt):
        ...
        if self.blur is None:
            ...
            _, _, kh, kw = self.blur.shape
            assert kh == kw
            ksize = kh
            pca_size = opt["approx_basis"]
    ...
    def random_kernel(self):
        ...
        if r_value > 3:  # Motion blur
            ...
            k = self.extend_kernel_size(
                self.motion_ker[index_blur], (self.ksize, self.ksize)
            )
    ...
    def __getitem__(self, index):
        ...
        if self.sf == 3:
            H_patch = (
                self.patch_size - (self.patch_size % self.sf)
            ) + 48  # We add 20 pixels on each side of the image to avoid side effects for the blurring
        else:
        ...
        if self.blur is None:
            ...
            basis = torch.FloatTensor(
                np.array(
                    [
                        self.extend_kernel_size(
                            self.random_kernel(), (self.ksize, self.ksize)
                        )
                        for _ in range(self.pca_size)
                    ]
                )
            )
            ...
            for ann in anns:
                ...
                if mask.sum() > self.min_p_mask and mask_number < self.pca_size - 1:
                    ...
                    mask = ndimage.filters.convolve(
                        mask.astype(float), kernel, mode="wrap"
                    )
                    kernel_map[mask_number] = torch.FloatTensor(mask)

                    # Blur image on the given mask
                    blurred += ndimage.filters.convolve(
                        img_H, np.expand_dims(kernel, axis=2), mode="wrap"
                    ) * mask[:, :, np.newaxis].repeat(3, 2)
                    mask_number += 1
            ...
            # Blur mask
            mask_background = ndimage.filters.convolve(
                mask_background, kernel, mode="wrap"
            )
            ...
            blurred += ndimage.filters.convolve(
                img_H, np.expand_dims(kernel, axis=2), mode="wrap"
            ) * mask_background[:, :, np.newaxis].repeat(3, 2)
        else:
            ih, iw = img_H.shape[:2]
            gh, gw = self.blur.shape[:2]
            blurred = (
                svconv2d(
                    torch.FloatTensor(img_H).permute(2, 0, 1).unsqueeze(0),
                    F.interpolate(
                        torch.FloatTensor(self.blur)
                        .view(gh, gw, -1)
                        .permute(2, 0, 1)
                        .unsqueeze(0),
                        size=(iw, ih),
                    ...
                    .permute(0, 2, 3, 1)
                    .view(1, ih, iw, self.ksize, self.ksize)
                    .expand(3, -1, -1, -1, -1),
                    mode="circular",
                )
                .squeeze(0)
            ...
            basis, kernel_map = approximate(self.blur, basis_size=self.pca_size)

            basis = torch.FloatTensor(basis)
            kernel_map = torch.FloatTensor(kernel_map)

            kernel_map = F.interpolate(
        ...
        if self.blur is None:
            blurred /= (
                kernel_map.sum(axis=0)
                .view(1, H_patch - 48, H_patch - 48)
                .repeat(3, 1, 1)
            )
            kernel_map /= (
                kernel_map.sum(axis=0)
                .view(1, H_patch - 48, H_patch - 48)
                .repeat(self.pca_size, 1, 1)
            )
